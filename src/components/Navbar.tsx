...
const Navbar = ({ search = true }: { search?: boolean }) => {
  ...
  return (
    <header className="site-navbar sticky top-0 z-40 surface-dark border-b border-dark-border">
      <nav className="container-site h-14 flex items-center gap-6" aria-label="Main">
          ...
          {NAV.map((l) => (
            <Link
              ...
              className={cn(
                ...
                isActive(l.to) ? "text-dark-foreground font-medium" : "text-dark-muted hover:text-dark-foreground",
              )}
            >
              {l.label}
            </Link>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <AccountMenu className="navbar-account" />
        </div>

        {/* Mobile toggle */}
        <button
          type="button"
          className="md:hidden text-dark-foreground hover:text-primary p-1 rounded transition-colors"
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t border-dark-border surface-dark">
          <div className="container-site py-4 flex flex-col gap-3">
            ...
            {NAV.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className={cn("text-sm py-1", isActive(l.to) ? "text-dark-foreground font-medium" : "text-dark-muted hover:text-dark-foreground")}
              >
                {l.label}
              </Link>
            ))}
            <a href={GITHUB} target="_blank" rel="noopener noreferrer" className="text-sm text-dark-muted hover:text-dark-foreground py-1 inline-flex items-center gap-2">
              <Github size={15} /> GitHub
            </a>
            <div className="border-t border-dark-border pt-3">
              <AccountMenu className="navbar-account text-sm" variant="menu" />
            </div>
          </div>
        </div>
      )}

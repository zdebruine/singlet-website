...
const ProductHub = () => {
  ...
  const createProject = async (e: FormEvent) => { e.preventDefault(); setBusy(true); setError(""); try { const r = await apiClient.product.createProject(project); const p = r.project as { id: string }; if (typeof r.read_token === "string") sessionStorage.setItem(`singlet.project-token.${p.id}`, r.read_token); window.location.assign(`/projects/${p.id}`); } catch (x) { setError(isApiError(x) ? x.message : "Could not create project."); } finally { setBusy(false); } };

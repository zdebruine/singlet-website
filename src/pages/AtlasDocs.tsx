...
const AtlasDocs = () => {
  ...
  const totalSamples = stats?.total_samples?.toLocaleString() ?? "4,500+";
  const successSamples = stats?.success_samples?.toLocaleString() ?? "1,975+";
  const totalCells = stats?.total_cells ? (stats.total_cells / 1e6).toFixed(1) + "M" : "5.4M";
  const seriesCount = stats?.series_count?.toLocaleString() ?? "1,679+";
...
# → singlet_stats → "4,500+ samples (1,975 SUCCESS), 5.4M cells, 8 species"
